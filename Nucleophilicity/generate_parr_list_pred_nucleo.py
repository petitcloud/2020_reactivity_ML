...
# exp_elec = []
 ...
 for a in range(len(indexline)):
  ...
  index_value.append(float(indexline[a].split()[1]))
...
# exp_elec = numpy.array(exp_elec)
 ...
 for b in range(len(folder_name)):
  tempfile = pd.read_excel('{0}/{1}/ie_adia/Parr.xlsx' .format(dirc, folder_name[b]), 'Sheet1')
...
if __name__ == '__main__':
 ...
 print ('<folder_name>\t<ie*p1>\t<pdf>\t')
 for d in range(len(folder_name)):
  print (folder_name[d], (parr_mat[d]), log10(sum(parr_ind_mat[d])))
